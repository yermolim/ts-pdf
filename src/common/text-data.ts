import { DomUtils , Quadruple } from "ts-viewers-core";
...
export class TextData {
  ...
  static async buildAsync(text: string, options: TextDataOptions): Promise<TextData> {
    ...
    if (text) {
      ...
      for (let i = 0; i < words.length; i++) {
        ...
        await DomUtils.runEmptyTimeout(); // allow DOM to redraw to recalculate dimensions
