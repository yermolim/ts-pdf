...
export class FlateDecoder {
  ...
  private static removePngFilter(input: Uint8Array, 
    ...
    // const lineMax = columns - 1; 
    // const getLeft = (i: number) => 
    //   (i % lineLen_filtered) < 2 // the element is the first one in the line, so 'left' is 0
    //     ? 0 
    //     : output[((i - 2) % lineLen) + Math.floor(i / lineLen_filtered) * lineMax];

    // const getAboveIndex = (i: number) => 
    //   i - (lineLen_filtered + Math.floor(i / lineLen_filtered));
       
    // let aboveIndex = 0;     
    // const getAbove = (i: number) => {
    //   aboveIndex = getAboveIndex(i);
    //   return aboveIndex < 0 // the above index is negative, so the element is in the first line, so 'above' is 0
    //     ? 0 
    //     : output[aboveIndex];
    // };    
    // const getUpperLeft = (i: number) => 
    //   // assume that getUpperLeft always called after getAbove, 
    //   // so aboveIndex should be already calculated
    //   (aboveIndex < 1) // the above index is 0 or negative, so 'upperLeft' is 0
    //     ? 0 
    //     : output[aboveIndex - 1];
